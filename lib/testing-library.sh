...
run_tests() {
    ...
    case ${THUNDER_TRAVIS_TEST_RUNNER} in
        "phpunit")
            php ${THUNDER_TRAVIS_DRUPAL_INSTALLATION_DIRECTORY}/${composer_bin_dir}/phpunit --verbose --debug -c ${THUNDER_TRAVIS_DRUPAL_INSTALLATION_DIRECTORY}/${docroot}/core ${THUNDER_TRAVIS_DRUPAL_INSTALLATION_DIRECTORY}/${docroot}/modules/contrib/${THUNDER_TRAVIS_PROJECT_NAME} ${test_selection} || exit 1
        ;;
        "run-tests")
